...
> {
  ...
  render() {
    const { disabled, selections, getPopupContainer } = this.props;
    ...
    if (selections) {
      ...
      const menu = (
        <Menu className={styles[`selection-menu`]} selectedKeys={[]}>
      ...
      customSelections =
        newSelections.length > 0 ? (
          <Dropdown overlay={menu} getPopupContainer={getPopupContainer}>
            <div className={styles[`selection-down`]}>
              <Icon type="down" />
            </div>
          </Dropdown>
        ) : null;
      ...
      <div className={styles.selection}>
        <Checkbox
          className={classNames({
            [styles[`selection-select-all-custom`]]: customSelections,
          } as ClassDictionary)}
          checked={checked}
          indeterminate={indeterminate}
          disabled={disabled}
          onChange={
            (this.handleSelectAllChagne as any) as React.ChangeEventHandler<HTMLInputElement>
          }
        />
        {customSelections}
      </div>
