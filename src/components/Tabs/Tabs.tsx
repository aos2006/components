...
import RcTabs, { TabPane as TabPaneComponent } from 'rc-tabs';
import ScrollableInkTabBar from 'rc-tabs/lib/ScrollableInkTabBar';
import TabContent from 'rc-tabs/lib/TabContent';
import classNames from 'classnames';
import Icon from '@latoken-component/icon';
import warning from 'antd/lib/_util/warning';
import isFlexSupported from 'antd/lib/_util/isFlexSupported';
...
export default class Tabs extends React.Component<TabsProps, any> {
  static TabPane = TabPaneComponent as React.ClassicComponentClass<TabPaneProps>;
  ...
  render() {
    ...
    const cls = classNames(className, 'lat-tabs', styles.tabs, {
      [styles.vertical]: tabPosition === 'left' || tabPosition === 'right',
      [styles[size]]: !!size,
      [styles.card]: type.indexOf('card') >= 0,
      [styles[type]]: true,
      [styles.noAnimation]: !tabPaneAnimated,
    });
    ...
    if (type === 'editable-card') {
        ...
        (child: React.ReactElement<any>, index) => {
          ...
          childrenWithClose.push(
            React.cloneElement(child, {
              tab: (
                <div className={closable ? undefined : styles[`tab-unclosable`]}>
                  <span>{child.props.tab}</span>
                  {closeIcon}
                </div>
              ),
              key: child.key || index,
      ...
      if (!hideAdd) {
        tabBarExtraContent = (
          <span>
            <Icon glyph="plus" className={styles[`new-tab`]} onClick={this.createNewTab} />
            {tabBarExtraContent}
          </span>
        );
      }
    ...
    tabBarExtraContent = tabBarExtraContent ? (
      <div className={styles[`extra-content`]}>{tabBarExtraContent}</div>
    ) : null;
